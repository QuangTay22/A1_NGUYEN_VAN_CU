...
- Trang HTML đã gắn `app.js?v=8` để giảm nguy cơ bị cache JS cũ.
...
- Hãy hard refresh (`Ctrl + F5`) để trình duyệt lấy `app.js?v=8` mới nhất.
- Chạy lại `supabase.sql` để thêm default cho `title` và chuẩn hóa dữ liệu cũ.


## Nâng cấp giao diện mini social
- Trang `home.html` mới có: tiêu đề + nội dung + upload ảnh + upload mp3 (xem trước trước khi đăng).
- Có khu vực `Những ngày đặc biệt` cho CRUD cơ bản trên bảng `special_days`.
- Bài viết hiển thị dạng card, có ảnh/nhạc, có nút `Xem chi tiết` sang `detail.html`.
- Nếu muốn xem nhanh giao diện mà không cần đăng nhập (demo), mở `home.html?preview=1`.
